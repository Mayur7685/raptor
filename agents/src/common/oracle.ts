...
export interface OracleSnapshot { price: number; rawPrice: bigint; expo: number; ageSecs: number; }
...
export async function readOracleSnapshot(): Promise<OracleSnapshot | null> {
  try {
    ...
    return { price: Number(rawPrice) * Math.pow(10, expo), rawPrice, expo,
      ageSecs: Math.floor(Date.now() / 1000) - Number(item.price.publish_time) };
  } catch { return null; }
}
