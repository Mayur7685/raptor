...
export function useLiveBtcPrice(): LiveBtcPrice {
  const [state, setState] = useState<LiveBtcPrice>({ price: null, publishTimeSec: null, history: [] });
  const historyRef = useRef<PricePoint[]>([]);
  const model = useRef({ target: null as number | null, smoothed: null as number | null });

  useEffect(() => {
    let cancelled = false;
    let lastPublishTimeSec: number | null = null;
    ...
    const smoothId = setInterval(() => {
      ...
      trim(arr);
      setState({ price: m.smoothed, publishTimeSec: lastPublishTimeSec, history: arr.slice() });
    }, SMOOTH_MS);

    const setTarget = (price: number, publishTimeSec: number | null) => {
      lastPublishTimeSec = publishTimeSec;
      const m = model.current;
      m.target = price;
      if (m.smoothed == null) {
        m.smoothed = price;
        historyRef.current = seedPrebuffer(price);
        setState({ price, publishTimeSec, history: historyRef.current.slice() });
      }
    };
    ...
    const poll = async () => {
      ...
      try {
        ...
        const d = await r.json() as { price: number | null; publishTimeSec: number | null };
        if (d.price != null && Number.isFinite(d.price)) setTarget(d.price, d.publishTimeSec);
      } catch {}
    };
